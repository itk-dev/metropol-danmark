 ...
 */
function metropol_preprocess_page(&$vars, $hook) {
...
function phptemplate_preprocess_node(&$vars, $hook) {
  ...
  switch ($node->type) {
    case 'simplenews':
	//dsm($node);
	 $vars['month'] = $node->field_nyhedsbrev_dato[0]['view'];
 $vars['main_text'] = $node->content['body']['#value'];
 $main_image_path = $node->field_nyhedsbrev_image[0]['filepath'];
 $vars['main_image'] = theme('imagecache', 'nyhedsbrev', $main_image_path);
 $vars['vidst_du_text'] = $node->content['field_nyhedsbrev_vidste_du']['#children'];
	break;
  }
...
function metropol_theme() {
    return array(
     // The form ID.
     'simplenews_block_form_3' => array(
 ...
 function metropol_simplenews_block_form_3($form){
  $output = '';
  unset($form['subscriptions']['#description']);
  $form['mail']['#title'] =  t('Din e-mailaddresse');
  $form['submit']['#value'] =  t('Tilmeld');
